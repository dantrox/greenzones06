...
if( ! defined( 'GZ_VERSION' ) ) {
    define( 'GZ_VERSION', '0.1.0-dev' );
}
...
if( ! function_exists( 'url' ) ) {
     ...
     * @return string
     */
    function url( $path = '', $extra = array(), $secure = null )
    {
...
if( ! function_exists( 'array_sort_column' ) ) {
    ...
    function array_sort_column( $column, $arr, $sorting = SORT_ASC, $comparefn = 'strnatcasecmp' )
    {
        if( is_array( $arr ) && ( count( $arr ) > 0 ) ) {
            usort( $arr,
                function ( $a, $b ) use ( $comparefn, $column, $sorting ) {
...
if( ! function_exists( 'array_validate' ) ) {
    ...
    function array_validate( array $arr, array $validations )
    {
        ...
        foreach( $validations as $field => $validation ) {
            ...
            if( is_string( $validation ) and ( $validation{0} == '/' ) and ( preg_match( $validation,
                        $arr[ $field ]
                    ) == false )
            ) {
                $errors[] = $field;
...
if( ! function_exists( 'str_is_html' ) ) {
    ...
    function str_is_html( $str )
    {
        ...
        ) and ( preg_match( '/<\/?\w+((\s+\w+(\s*=\s*(?:".*?"|\'.*?\'|[^\'">\s]+))?)+\s*|\s*)\/?>/i', $str ) > 0 );
    }
}
